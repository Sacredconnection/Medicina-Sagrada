...
export function CartLink({ mobile = false }: { mobile?: boolean }) {
  ...
  return <Link href="/cart/" onClick={(event) => { if (!event.ctrlKey && !event.metaKey && !event.shiftKey && !event.altKey) { event.preventDefault(); openCart(); } }} aria-haspopup="dialog" className={mobile ? "mobile-cart-link" : "header-icon-button cart-link"} aria-label={`Sacola, ${count} ${count === 1 ? "item" : "itens"}`}>
